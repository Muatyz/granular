...
for j = 1:length(slip_tick)
    ...
    elseif type(j) == 0 %% 若类型为 micro-slip
        micro_slip_tick(end+1) = slip_tick(j);
    end
...
%%% micro_slip_pre_interval 内元素均为负数
micro_slip_pre_interval = [];
pointer_failure = 1;
for j = 1:length(micro_slip_tick)
    %%% 若参考的 failure tick 已在 micro-slip 之前, 更用下一个 failure tick
    if micro_slip_tick(j) >= failure_tick(pointer_failure)
        pointer_failure = pointer_failure + 1;
    end
    %%% 若可供参考的 failure tick 均已用完, 需要终止循环
    if pointer_failure > length(failure_tick)
        break
    end
    micro_slip_pre_interval(end+1) = micro_slip_tick(j) - failure_tick(pointer_failure);
end

%%% Plot the histogram of logN(occurrence)-preceding_time
figure(4)
interval = histogram(micro_slip_pre_interval,nbins,'DisplayStyle','stairs');
ax = gca;
set(gca,'yscale','log')
view(ax,[90,-90]); %% 交换 x-y 轴位置
xlabel("Preceding Time of Micro-Slips")
xlim([min(micro_slip_pre_interval),0]);
ylabel("log N_{occurrence}")
title("Histogram of Preceding Time about Micro-Slip Events before Failure")
