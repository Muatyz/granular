...
%%%
%%% Note. Actually the function can work at any length(Amp_2) >
%%% length(time_window)
%%%
%%% **index_1** is defined as index of time of the wave_front **Amp_1**, 
%%% which should be observed at the first place. 
%%% Please spot it at the first place.
%%%
%%% **time_window** is defined as the typical wave train length, which should 
%%% be observed at the first place. A typical time_window is about 1 ms.
...
%%% Amp_2
%%% 
%%% Actually Amp_1 could be the latter signal and Amp_2 the former signal.
%%% Use inverse order to complete the calculation process.
%%%
...
for n = 1 : length(index_2_m)
    wave_2 = Amp_2(n : (n + time_window_length - 1));
    coeffi_m(n) = sum(wave_1 .* wave_2)/ (sqrt(sum(wave_1.^2)) * sqrt(sum(wave_2.^2)));
end
...
[coefficient, index_2] = max(real(coeffi_m));
end